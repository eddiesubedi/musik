...
function index2(data2, key) {
  if (data2 instanceof Dict) {
    ...
    return new Ok2(result.isOk() ? new Some(result[0]) : new None);
  }
  ...
  if (key_is_int && Array.isArray(data2) || data2 && typeof data2 === "object" || data2 && Object.getPrototypeOf(data2) === Object.prototype) {
    if (key in data2)
      return new Ok2(new Some(data2[key]));
    return new Ok2(new None);
  }
...
class Anime extends CustomType {
  constructor(id, title2, episodes) {
    super();
    this.id = id;
    this.title = title2;
...
function anime_decorder() {
  return field("id", int2, (id) => {
    return field("title", string2, (title2) => {
      return field("episodes", int2, (episodes) => {
        return success(new Anime(id, title2, episodes));
      });
    });
  });
}
function anime_list_decorder() {
  return list2(anime_decorder());
}
// build/dev/javascript/client/client.mjs
var FILEPATH = "src/client.gleam";

class Model extends CustomType {
  constructor(anime, loading) {
    super();
    this.anime = anime;
...
class ApiReturnedAnime extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
class UserClickedRefresh extends CustomType {
}
function fetch_anime() {
  return get3("/api/anime", expect_json(anime_list_decorder(), (var0) => {
    return new ApiReturnedAnime(var0);
  }));
}
function init(flags) {
  if (flags instanceof Empty) {
    return [new Model(toList([]), true), fetch_anime()];
  } else {
    return [new Model(flags, false), none()];
  }
}
function update2(model, msg) {
  if (msg instanceof ApiReturnedAnime) {
    let $ = msg[0];
    if ($ instanceof Ok2) {
      let anime = $[0];
      return [new Model(anime, false), none()];
    } else {
      return [new Model(model.anime, false), none()];
    }
  } else {
    return [new Model(model.anime, true), fetch_anime()];
  }
}
function view_anime(anime) {
  return li(toList([
    styles(toList([
      ["padding", "0.75rem"],
      ["margin", "0.5rem 0"],
    ...
    strong(toList([]), toList([text3(anime.title)])),
    text3(" - " + to_string(anime.episodes) + " episodes")
  ]));
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Anime Tracking")])),
    button(toList([
      on_click(new UserClickedRefresh),
    ...
    (() => {
      let $ = model.anime;
      if ($ instanceof Empty) {
        return p(toList([]), toList([text3("No anime loaded.")]));
      } else {
        let items = $;
        return ul(toList([
          styles(toList([["list-style", "none"], ["padding", "0"]]))
        ]), map2(items, view_anime));
      }
    })()
  ]));
}
function main() {
  ...
  _block = try$(_pipe$1, (text4) => {
    let _pipe$22 = parse(text4, anime_list_decorder());
    return replace_error(_pipe$22, undefined);
  });
  echo(_block, undefined, "src/client.gleam", 17);
  let _block$1;
  let _pipe$2 = querySelector("#model");
  let _pipe$3 = map4(_pipe$2, innerText);
  let _pipe$4 = try$(_pipe$3, (text4) => {
    let _pipe$42 = parse(text4, anime_list_decorder());
    return replace_error(_pipe$42, undefined);
  });
  _block$1 = unwrap2(_pipe$4, toList([]));
  let hydrate_anime = _block$1;
  echo(hydrate_anime, undefined, "src/client.gleam", 30);
  let app = application(init, update2, view);
  let $ = start4(app, "#app", hydrate_anime);
  if (!($ instanceof Ok2)) {
    throw makeError("let_assert", FILEPATH, "client", 32, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 889, end: 948, pattern_start: 900, pattern_end: 905 });
  }
  return;
}
...
class Echo$Inspector {
  ...
  #dict(map9) {
    ...
    let first = true;
    let key_value_pairs = fold(map9, [], (pairs, key2, value2) => {
    ...
    key_value_pairs.forEach(([key2, value2]) => {
      if (!first)
        body2 = body2 + ", ";
      body2 = body2 + "#(" + this.inspect(key2) + ", " + this.inspect(value2) + ")";
      first = false;
    });
    return body2 + "])";
  }
